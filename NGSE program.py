...
# Plot the best routing paths on a map
fig = plt.figure()
...
ax.scatter(*zip(*lhr), c='r', label='LHR (Heathrow Airport)')
ax.scatter(*zip(*ewr), c='b', label='EWR (Newark Liberty International Airport)')
...
for idx, best_individual_indices_tuple in enumerate(best_individuals):
    ...
    for i in best_individual_indices:
        if isinstance(i, int) and 0 <= i < len(dataset):
            path = convert_to_cartesian([float(dataset[i][3])], [float(dataset[i][4])], [float(dataset[i][2])])
            routing_path.extend(path)
    if routing_path:
        xs, ys, zs = zip(*routing_path)
        ax.plot(xs, ys, zs, label=f'Routing Path {idx+1}')
