...
def max_data_flow(P1=random.choice(rc), P2=LHR):
    ...
    for plane in rc:
        if plane not in datapath:
            if haversine_distance(P1, plane)+haversine_distance(plane, P2)<=minimum_dist and TRLD(haversine_distance(P1, P2))[0]<TRLD(haversine_distance(plane, P2))[0]:
                minimum_dist = haversine_distance(plane, P2)
                planetobeadded = plane
                print(minimum_dist,planetobeadded)
    if planetobeadded==[]:
        return datapath
    m=datapath[-1]
    datapath[-1]=planetobeadded
    datapath.append(m)
    return max_data_flow(P1,planetobeadded)
